...
from pyknow import TEST, AND, OR, NOT, Rule, Fact, KnowledgeEngine, W
...
class Bipolar(KnowledgeEngine):
    ...
    @Rule(TEST(somatorio([
        Sintomas().get("autoestima_excessiva"),
        Sintomas().get("reducao_sono"),
        Sintomas().get("aumento_fala"),
        Sintomas().get("mudanca_modo_pensar"),
        Sintomas().get("distrabilidade"),
        Sintomas().get("agitacao"),
        Sintomas().get("envolvimento_atividade_risco")
    ]) >= 3))
    ...
    @Rule(AND(OR(Sintomas(humor_deprimido=1), Sintomas(perda_interesse=1)), TEST(somatorio([
        Sintomas().get("humor_deprimido"),
        Sintomas().get("perda_interesse"),
        Sintomas().get("alteracao_alimentacao"),
        Sintomas().get("alteracao_sono"),
        Sintomas().get("alteracao_comportamentao"),
        Sintomas().get("cansaco"),
        Sintomas().get("sentimento_depressivo"),
        Sintomas().get("alteracao_pensamento"),
        Sintomas().get("pensamentos_morte")
    ]) >= 5)))
